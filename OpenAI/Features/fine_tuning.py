...
def main(argv: Optional[list[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Fine-tuning: jobs create/list")
    sub = p.add_subparsers(dest="action", required=True)

    pc = sub.add_parser("create", help="ジョブ作成")
    pc.add_argument("--training-file", required=True, help="files API でアップロード済みの file-id (JSONL)")
    pc.add_argument("--model", required=True, help="微調整対象モデル (例: gpt-4o-mini)")
