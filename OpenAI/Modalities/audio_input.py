...
def main(argv: Optional[list[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Modalities: Audio (Input only)")
    p.add_argument("--file", required=True, help="音声ファイルパス (mp3/mp4/mpeg/mpga/m4a/wav/webm など)")
    p.add_argument(
        "--task",
    ...
    p.add_argument("--model", default="whisper-1", help="音声モデル (例: whisper-1 / gpt-4o-mini-transcribe)")
    ...
    if args.dry_run:
        preview = {
            "audio": {"file": os.path.basename(args.file)},
            "task": args.task,
                ...
                "messages": [
                    {"role": "system", "content": args.system},
                    {
                        ...
                        "content": (
                            ...
                            if args.task == "qa"
                            else "次の文字起こしを日本語で簡潔に要約してください: <transcript here>"
                        ),
                    },
                ],
        ...
        print("[DRY-RUN] audio flow preview:")
    ...
    # 2) summarize/qa の場合は Chat へ
    if args.task in {"summarize", "qa"}:
        messages = [{"role": "system", "content": args.system}]
        if args.task == "summarize":
            user_content = f"次の文字起こしを日本語で簡潔に要約してください:\n\n{transcript}"
        else:  # qa
            user_content = (
                (args.prompt or "この音声内容に関して有用な要点を挙げてください。")
                + "\n\n[文字起こし]\n" + transcript
            )
        ...
        # transcribe/translate は文字起こし結果をそのまま出力
